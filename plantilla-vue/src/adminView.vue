        ...
        <md-tabs md-sync-route class="md-transparent" md-alignment="fixed">
            <md-tab id="tab-home" md-label="Resumen" to="/components/tabs/home">
                ...
                <md-card style="width: 45%; float: right">
                    ...
                    <md-card style="width: 45%; float: right">
                        <md-card-header>
                            <div class="md-title">Cantidades por Nivel de Estudios:</div>
                        </md-card-header>
                        <md-card-content>
                            <div>
                                <vue-chart  v-if="this.tortaEstudios !== null" type="pie" :data="this.tortaEstudios"></vue-chart>
                                <div v-else>
                                    <div class=" lds-css ng-scope">
                                        <div style="width:100%;height:100%" class="lds-bars">
                                            <div></div><div></div><div></div><div></div> <div></div>
                    ...
                    <md-card style="width: 45%; float: left">
                        <md-card-header>
                            <div class="md-title">Cantidades por Nivel de Ingresos:</div>
                        </md-card-header>
                        <md-card-content>
                            <div>
                                <vue-chart  v-if="this.tortaNivelIngresos !== null" type="pie" :data="this.tortaNivelIngresos"></vue-chart>
                                <div v-else>
                                    <div class=" lds-css ng-scope">
                                        <div style="width:100%;height:100%" class="lds-bars">
                                            <div></div><div></div><div></div><div></div> <div></div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            crearTortaNivelEstudios(){
                let dataNivelEstudios = []
        let contadorBI = 0;
                let contadorBC = 0; let contadorMI = 0; let contadorMC = 0;
                let contadorTI = 0; let contadorTC = 0; let contadorUI = 0;
                let contadorUC = 0; let contadorEPI = 0; let contadorEPC = 0;
                let contadorO = 0; let contadorNA = 0;
                for (let i=0; i<this.dataEncuestados.length; i++){
                    if (this.dataEncuestados[i].nivelDeEstudio == "Básica incompleta"){
                        contadorBI++
                        console.log("la cantidad de bc", contadorBI)
                    }
                   else if (this.dataEncuestados[i].nivelDeEstudio == "Básica completa"){
                        contadorBC++
                        console.log("la cantidad de bc", contadorBC)
                    }
                    else if(this.dataEncuestados[i].nivelDeEstudio == "Media incompleta"){
                        contadorMI++
                        console.log("lac antidad de mujer es", contadorMI)
                    }
                    else if(this.dataEncuestados[i].nivelDeEstudio == "Media completa"){
                        contadorMC++
                        console.log("lac antidad de mujer es", contadorMC)
                    }
                    else if(this.dataEncuestados[i].nivelDeEstudio == "Técnico (o instituto profesional) incompleto"){
                        contadorTI++
                        console.log("lac antidad de mujer es", contadorTI)
                    }
                    else if(this.dataEncuestados[i].nivelDeEstudio == "Técnico (o instituto profesional) completo"){
                        contadorTC++
                        console.log("lac antidad de mujer es", contadorTC)
                    }
                    else if(this.dataEncuestados[i].nivelDeEstudio == "Universitario incompleto"){
                        contadorUI++
                        console.log("lac antidad de mujer es", contadorUI)
                    }
                    else if(this.dataEncuestados[i].nivelDeEstudio == "Universitario completo"){
                        contadorUC++
                        console.log("lac antidad de mujer es", contadorUC)
                    }
                    else if(this.dataEncuestados[i].nivelDeEstudio == "Estudios de postgrado incompleto"){
                        contadorEPI++
                        console.log("lac antidad de mujer es", contadorEPI)
                    }
                    else if(this.dataEncuestados[i].nivelDeEstudio == "Estudios de postgrado completo"){
                        contadorEPC++
                        console.log("lac antidad de mujer es", contadorEPC)
                    }
                    else if(this.dataEncuestados[i].nivelDeEstudio == "Otro"){
                        contadorO++
                        console.log("lac antidad de mujer es", contadorO)
                    }
                    else {
                        contadorNA++
                    }
                }
                dataNivelEstudios.push(contadorBI);
                dataNivelEstudios.push(contadorBC);
                dataNivelEstudios.push(contadorMI);
                dataNivelEstudios.push(contadorMC);
                dataNivelEstudios.push(contadorTI);
                dataNivelEstudios.push(contadorTC);
                dataNivelEstudios.push(contadorUI);
                dataNivelEstudios.push(contadorUC);
                dataNivelEstudios.push(contadorEPI);
                dataNivelEstudios.push(contadorEPC);
                dataNivelEstudios.push(contadorO);
                dataNivelEstudios.push(contadorNA);
                console.log("la data queda en el orden:", dataNivelEstudios)
                this.tortaEstudios = {
                    labels: ["Básica incompleta",
                        "Básica completa",
                        "Media incompleta",
                        "Media completa",
                        ...
                        "Estudios de postgrado incompleto",
                        "Estudios de postgrado completo",
                        "Otro",
                        "No sabe-No Responde"],
                    datasets: [
                        {
                            ...
                            backgroundColor:  ['#FF7043', '#FFA726', '#FFCA28', '#FFEE58', '#D4E157', '#9CCC65 ',
                                '#26A69A'],
                            data: dataNivelEstudios
                        },
                    ]
                }
            },

            crearTortaNivelIngresos(){
                let dataNivelIngresos = []
                let contador1 = 0; let contador2 = 0; let contador3 = 0;
                let contador4 = 0; let contador5 = 0; let contador6 = 0;
                let contador7 = 0; let contador8 = 0; let contador9 = 0;
                let contadorNA = 0;
                for (let i=0; i<this.dataEncuestados.length; i++){
                    if (this.dataEncuestados[i].nivelDeIngreso == "Menos de 150.000"){
                        contador1++
                        console.log("la cantidad de hombres es", contador1)
                    }
                    else if(this.dataEncuestados[i].nivelDeIngreso == "Entre 150.001 y 250.000"){
                        contador2++
                        console.log("lac antidad de mujer es", contador2)
                    }
                    else if(this.dataEncuestados[i].nivelDeIngreso == "Entre 250.001 y 450.000"){
                        contador3++
                        console.log("lac antidad de mujer es", contador3)
                    }
                    else if(this.dataEncuestados[i].nivelDeIngreso == "Entre 450.001 y 750.000"){
                        contador4++
                        console.log("lac antidad de mujer es", contador4)
                    }
                    else if(this.dataEncuestados[i].nivelDeIngreso == "Entre 750.001 y 1.000.000"){
                        contador5++
                        console.log("lac antidad de mujer es", contador5)
                    }
                    else if(this.dataEncuestados[i].nivelDeIngreso == "Entre 1.000.001 y 1.500.000"){
                        contador6++
                        console.log("lac antidad de mujer es", contador6)
                    }
                    else if(this.dataEncuestados[i].nivelDeIngreso == "Entre 1.500.001 y 2.000.000"){
                        contador7++
                        console.log("lac antidad de mujer es", contador7)
                    }
                    else if(this.dataEncuestados[i].nivelDeIngreso == "Entre 2.000.001 y 3.000.000"){
                        contador8++
                        console.log("lac antidad de mujer es", contador8)
                    }
                    else if(this.dataEncuestados[i].nivelDeIngreso == "Más de 3.000.001"){
                        contador9++
                        console.log("lac antidad de mujer es", contador9)
                    }
                    else {
                        contadorNA++
                    }
                }
                dataNivelIngresos.push(contador1);
                dataNivelIngresos.push(contador2);
                dataNivelIngresos.push(contador3);
                dataNivelIngresos.push(contador4);
                dataNivelIngresos.push(contador5);
                dataNivelIngresos.push(contador6);
                dataNivelIngresos.push(contador7);
                dataNivelIngresos.push(contador8);
                dataNivelIngresos.push(contador9);
                dataNivelIngresos.push(contadorNA);
                console.log("la data queda en el orden:", dataNivelIngresos)
                this.tortaNivelIngresos = {
                    labels: ["Menos de 150.000",
